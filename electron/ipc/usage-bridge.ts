 ...
 * `pricing` returns the config-driven pricing status (priced + unpriced, M10.A) for Settings. Pure +
 * transport-agnostic so the mapping is Node-unit-testable, leaving preload.ts thin. No key, no DB
...
export function createUsageApi(invoke: IpcInvoke): UsageApi {
  return {
    ...
    pricing: () => invoke(USAGE_CHANNELS.pricing) as Promise<PricingStatus>,
  };
}
